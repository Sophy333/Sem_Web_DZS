...
# find contradiction id too
contradiction_id = None
for name, idx in label2id.items():
    if "contrad" in name.lower():
        contradiction_id = idx
        break
if contradiction_id is None:
    contradiction_id = 0  # common fallback
...
def predict_zero_shot_batch(batch, candidate_labels, hypo_template):
    ...
    score = logits[:, :, entailment_id] - logits[:, :, contradiction_id]
    best_idx = torch.argmax(score, dim=-1).cpu().tolist()
    # logits = logits.view(num_examples, num_labels, -1)
...
print("Gold distribution:", Counter(all_true))
